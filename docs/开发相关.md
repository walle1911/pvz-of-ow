...
开发者模式的“数值调整”会先保存到 `开发者包/numerical_adjustments.json`，便于试玩和反复调整。导出程序把 `存档/` 和 `开发者包/` 放在游戏旁；编辑器运行使用 `user://` 下的同名分类。确认数值后必须把它烘焙进角色场景，才能作为项目源码随 Git 和导出包分发。

- 单个角色：从 Godot 编辑器运行开发者模式，进入角色数值页，点击“烘焙到 .tscn”。
- 全部角色：使用 Godot 编辑器右侧“植物数值白名单”面板底部的“将开发者数值烘焙到角色 .tscn”。
- 面板中的“待烘焙/已烘焙”清单可展开到具体节点、字段和值；批量写入时会逐角色显示进度。
- 已烘焙记录保存在 `res://data/numerical_bake_manifest.json`，应与对应 `.tscn` 一起提交。
- 烘焙器只在编辑器环境开放；导出应用不能写入 PCK 内的 `res://`。
- 烘焙成功后，对应的 `user://` 临时覆盖会被清除，实际权威值由角色 `.tscn` 提供。
- 植物阳光、冷却和僵尸刷新权重也拥有场景字段，会一起写入角色根节点。

烘焙后先检查 Git diff，再运行 Godot headless 校验；不要把 `user://` 文件当作正式发布数据。
