...
func add_item(item_data: Variant, item_weight: float, rebuild: bool = true, check_repeat:bool= true) -> bool:
	if item_weight <= 0:
		printerr("道具权重必须为正数,道具: ", item_data, "权重:", item_weight)
		return false
	if check_repeat:
		if has_item(item_data):
			print("要添加的物品已存在，无法添加", {"item_data": item_data})
			return false
	...
	for i in range(_item_pool.size()):
		if _item_pool[i].data == item_data:
			...
			if rebuild:
				_build_alias_table()
			return true
	print("要删除的物品不存在", {"item_data": item_data})
	return false
...
func update_item_weight(item_data: Variant, new_weight: float, rebuild: bool = true) -> bool:
	if new_weight <= 0:
		printerr("权重必须为正数：%s" % str(item_data))
		return false
	for i in range(_item_pool.size()):
		if _item_pool[i].data == item_data:
			...
			if rebuild:
				_build_alias_table()
			return true
	print("要更新的物品不存在：%s" % str(item_data))
	return false
...
func get_item_weight(item_data: Variant) -> float:
	for item in _item_pool:
		if item.data == item_data:
			return item.weight
	print("要获取权重的物品不存在", {"item_data": item_data})
	return -1.0
...
func get_items_weights(item_datas: Array) -> Dictionary:
	...
	for missing in target_set:
		print("要获取权重的物品不存在", {"item_data": missing})
	return result
