...
func update_multi_round_zombie_refresh_types(curr_round:int, game_sences:MainSceneRegistry.MainScenes) -> void:
	...
	if curr_round == 1:
		zombie_refresh_types.append(CharacterRegistry.ZombieType.Z501Norm)
		zombie_refresh_types.append(CharacterRegistry.ZombieType.Z503Cone)
		zombie_refresh_types.append(CharacterRegistry.ZombieType.Z505Bucket)
	else:
		...
		zombie_refresh_types.append(CharacterRegistry.ZombieType.Z501Norm)
		whitelist_refresh_zombie_types_copy.erase(CharacterRegistry.ZombieType.Z501Norm)
		# 第二种：80% 路障 (Cone)，20% 报纸 (Paper)
		var prob = randf()
		if prob < 0.8:
			zombie_refresh_types.append(CharacterRegistry.ZombieType.Z503Cone)
			whitelist_refresh_zombie_types_copy.erase(CharacterRegistry.ZombieType.Z503Cone)
		else:
			zombie_refresh_types.append(CharacterRegistry.ZombieType.Z506Paper)
			whitelist_refresh_zombie_types_copy.erase(CharacterRegistry.ZombieType.Z506Paper)
		...
		for i in range(min(curr_round * 2, 8)):
			...
			if zombie_type_choose == CharacterRegistry.ZombieType.Z521Bungi:
	...
	if is_bungi:
		print(Global.character_registry.get_zombie_info(CharacterRegistry.ZombieType.Z521Bungi, CharacterRegistry.ZombieInfoAttribute.ZombieName))
