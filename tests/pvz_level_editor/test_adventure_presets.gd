...
func _test_card_unlock_curve() -> void:
	...
	_expect(night_first.has(43) and not night_first.has(14), "2-1 应已解锁莫伊拉但尚未解锁黑百合")
	var night_second: Array = Presets.build_level("adventure_2_2").get("availablePlants", [])
	_expect(night_second.has(43) and night_second.has(14), "2-2 应同时拥有莫伊拉与 2-1 奖励黑百合")
	_expect(Presets.build_level("adventure_1_10").get("rewardPlants", []) == [43], "1-10 奖励应为莫伊拉")
	_expect(Presets.build_level("adventure_2_1").get("rewardPlants", []) == [14], "2-1 奖励应为黑百合")
