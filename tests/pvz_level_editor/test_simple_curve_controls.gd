...
func _ready() -> void:
	...
	assert(int(level_1_3_para.simple_zombie_intro_waves[CharacterRegistry.ZombieType.Z504Bucket]) == 6)
	...
	assert(not before_intro.has(CharacterRegistry.ZombieType.Z504Bucket))
	var middle_intro: Array[CharacterRegistry.ZombieType] = level_1_3_creator.create_curr_wave_zombie_list(5, false)
	assert(middle_intro.has(CharacterRegistry.ZombieType.Z504Bucket))
	assert(level_1_3_creator.zombie_weights[CharacterRegistry.ZombieType.Z500Norm] < 4000)
	assert(level_1_3_creator.zombie_weights[CharacterRegistry.ZombieType.Z502Cone] < 4000)
	...
	assert(all_planned_types.has(int(CharacterRegistry.ZombieType.Z500Norm)))
	assert(all_planned_types.has(int(CharacterRegistry.ZombieType.Z502Cone)))
	assert(all_planned_types.has(int(CharacterRegistry.ZombieType.Z504Bucket)))
	var level_1_3_bucket_index := level_1_3_types.find(CharacterRegistry.ZombieType.Z504Bucket)
	...
	missing_level["simpleBaseZombieType"] = int(CharacterRegistry.ZombieType.Z500Norm)
	missing_level["simpleFlagZombieType"] = int(CharacterRegistry.ZombieType.Z501Flag)
	missing_level["simpleZombiePool"] = [
		int(CharacterRegistry.ZombieType.Z500Norm),
		int(CharacterRegistry.ZombieType.Z523Gargantuar),
	]
	...
	for wave_index in range(0, 9):
		assert(not missing_creator.simple_wave_plan[wave_index].has(CharacterRegistry.ZombieType.Z523Gargantuar))
	assert(missing_creator.simple_wave_plan[9].has(CharacterRegistry.ZombieType.Z523Gargantuar))
